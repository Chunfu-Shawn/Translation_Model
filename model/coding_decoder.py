...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, n_heads, p_drop):
        ...
        self.self_attention = FlashMultiHeadedAttention(d_model, n_heads, p_drop, causal=True)
        self.cross_attention = FlashMultiHeadedAttention(d_model, n_heads, p_drop, causal=False)
    ...
    def incremental_step(self,
        ...
        out_self, self_k_cache, self_v_cache, self_cache_seqlens = self.self_attention.decode_step_with_kvcache(
        ...
        if src_kv_per_layer is None:
            ...
            if isinstance(src_kv_per_layer, tuple) and len(src_kv_per_layer) == 3 and src_kv_per_layer[0].dim() == 4 and src_kv_per_layer[0].shape[0] == bs:
                ...
                q = self.cross_attention.toqueries(
                    out_self).view(bs, 1, 
...
class Decoder(nn.Module):
    ...
    # produce encoder caches in kvcache layout for use with flash kvcache cross-attend
    ...
    def incremental_decode(self,
        ...
        for _ in range(max_steps):
            ...
            for li, layer in enumerate(self.decoder_layers):
                ...
                x_embs, k_cache, v_cache, cache_seqlen = layer.incremental_step(x, src_kv, k_cache, v_cache, cache_seqlen)
                k_caches[li] = k_cache
                v_caches[li] = v_cache
                cache_seqlens[li] = cache_seqlen

            # x_embs is (bs, 1, d_model)
            cur_input = x_embs
            outputs.append(x_embs)

        seq_embs = torch.cat(outputs, dim=1)  # (bs, max_steps, d_model)
        return seq_embs
...
class CodingDecoder(nn.Module):
    ...
    def generate(self,
        ...
        seq_embs = self.decoder.incremental_decode(start_emb,
