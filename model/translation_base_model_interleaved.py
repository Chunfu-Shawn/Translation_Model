...
from model.model_modules import HeadAdapter, AddPromptEmbedding, InterleavedEncoder, InterleavedEncoderLayer
...
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# NEW: Main Model with Interleaved Architecture
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
class TranslationBaseModel(nn.Module):
    ...
    def __init__(
        ...
        # --- MODIFICATION 1: Separate Modality Embeddings ---
        # Instead of a single fusion embedding layer, we create separate
        # linear projection layers for RNA sequence and P-site counts.
        self.rna_emb = nn.Linear(d_seq, d_model)
        self.psite_emb = nn.Linear(d_count, d_model)

        # cell type handling
        self.num_cells = len(all_cell_types)
        # mapping: token string -> index [0..num_cells-1]
        self.cell_type_mapping = dict(zip(all_cell_types, range(self.num_cells)))
        ...
        # --- MODIFICATION 2: Use Interleaved Encoder ---
        # Instantiate and use the new InterleavedEncoderLayer and InterleavedEncoder.
        interleaved_layer = InterleavedEncoderLayer(d_model, d_ff, n_heads, p_drop)
        self.encoder = InterleavedEncoder(interleaved_layer, number_of_layers)
    ...
    def forward(
        ...
        **kwargs
        ...
        if src_mask is not None:
            ...
            src_mask = torch.ones(bs, seq_len, dtype=torch.bool, device=device)
        ...
        if cell_type_idx.dim() != 1 or cell_type_idx.shape[0] != bs:
         ...
         # --- MODIFICATION 3: Separate Embedding and Prompt Handling ---
        # 1. Embed RNA and P-site data into separate streams.
        rna_embs = self.rna_emb(seq_batch)      # (bs, seq_len, d_model)
        psite_embs = self.psite_emb(count_batch)  # (bs, seq_len, d_model)

        # 2. Add cell prompt embedding ONLY to the P-site stream.
        # This conditions the "query" stream with cell-type information.
        psite_embs_prompted, psite_mask = self.add_prompt_emb(psite_embs, src_mask, cell_type_idx)
        # -> psite_embs_prompted shape: (bs, pmt_len + seq_len, d_model)
        # -> psite_mask shape: (bs, pmt_len + seq_len)

        # 3. Pad the RNA stream to match the length of the prompted P-site stream.
        # The mask for these padded tokens will be False (invalid), so they are ignored
        # by the attention mechanism.
        if self.pmt_len > 0:
            prompt_pad = torch.zeros(bs, self.pmt_len, self.d_model, device=device)
            rna_embs_padded = torch.cat([prompt_pad, rna_embs], dim=1)
            
            # The mask for padded positions should be False (not valid)
            prompt_mask_pad = torch.zeros(bs, self.pmt_len, dtype=torch.bool, device=device)
            rna_mask = torch.cat([prompt_mask_pad, src_mask], dim=1)
        else:
            rna_embs_padded = rna_embs
            rna_mask = src_mask

        # --- MODIFICATION 4: Call the Interleaved Encoder ---
        # The encoder now takes both streams and their corresponding masks.
        # It returns the final, refined P-site representation.
        src_reps = self.encoder(
            rna_reps=rna_embs_padded,
            psite_reps=psite_embs_prompted,
            rna_mask=rna_mask,
            psite_mask=psite_mask
        )

        # --- Downstream Head Processing (unchanged) ---
        # The rest of the logic remains the same, operating on the final `src_reps`.
        if not head_names:
            return src_reps
        ...
        for name in head_names:
            ...
            outputs[name] = head(src_reps, src_mask=psite_mask, **per_head_kwargs)
