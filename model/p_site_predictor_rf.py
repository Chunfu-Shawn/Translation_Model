...
class PSitePredictor():
    ...
    def train_p_site_predictor(
        ...
        # 2) model initialize and train (RandomForest)
        rf_params = {
            "n_estimators": 500,
            "max_depth": None,                # you can set e.g. 12 to restrict complexity
            "min_samples_leaf": 3,
        ...
        # 3) predict & evaluate on validation (if available)
        if X_val is None or X_val.shape[0] == 0:
            ...
            try:
                ...
                # fallback: if predict fails, try argmax of predict_proba
                probs = self.predictor.predict_proba(X_val)
                y_pred = np.argmax(probs, axis=1)

            print("Accuracy:", accuracy_score(y_val, y_pred))
            print(classification_report(y_val, y_pred))
            cm = confusion_matrix(y_val, y_pred)
            pd.DataFrame(cm, index=range(offset_min, offset_min + np.max(y_val) + 1), 
                        columns=range(offset_min, offset_min + np.max(y_val) + 1)).to_csv(
                            os.path.join(output_dir, "confusion_matrix.csv"), header=True)
            print("Confusion matrix:\n", cm)

        # 4) save model
        model_path = os.path.join(output_dir, "rf_model.joblib")
        joblib.dump(self.predictor, model_path)
        print(f"RandomForest saved to {model_path}")
        ...
        with tqdm(total=total_tids, desc="Predicting P-site", mininterval=1) as pbar:
            while True:
                ...
                while start_idx < n_samples:
                    ...
                    offset_pred = self.predictor.predict(subX)  # 对子块一次预测
                    offsets = offset_pred + offset_limits[0]
...
if __name__ == "__main__":
    ...
    p_site_count = preditor.predict_read_specific_p_site(
        ...
        endonuclease = "MNase",
        output_dir = "/home/user/data3/rbase/translation_model/results/preprocess/WM902B/random_forest",
        mode = "train",
        train_ref = "TTS",
        offset_limits = (10, 14)
    )
