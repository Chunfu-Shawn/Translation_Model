...
class FlashMultiHeadedAttention(nn.Module):
    ...
    def decode_step_with_kvcache(self,
        ...
        # autoupdate k_cache and v_cache
    ...
    def build_kv_cache_from_kv(self,
        ...
        """
        # project to head space: (bs, seq_len, n_heads, head_dim) -> transpose to (bs, n_heads, seq_len, head_dim)
        ...
        # k_enc currently (bs, n_heads, src_len, head_dim) -> take [:, pos, :, :] and place into k_cache[bs, pos, n_heads, head_dim]
        k_trans = k_enc.permute(0, 2, 1, 3).contiguous()  # transpose to (bs, src_len, n_heads, head_dim)
    ...
    def cross_attend_using_kvcache(self,
        ...
        bs, q_len, _ = queries.shape

        # project q for cross-attn: shape (bs, n_heads, 1, head_dim)
        query = self.toqueries(queries).view(bs, 1, self.n_heads, self.head_dim).transpose(1,2)

        # apply RoPE for q using absolute position = new token position
        query = self.RoPE(query, positions=self_cache_seqlens).transpose(1,2)
