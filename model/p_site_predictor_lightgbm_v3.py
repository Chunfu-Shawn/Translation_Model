...
from eval.eval_RPF_density_TIS_TTS import build_length_position_matrix, build_length_frame_matrix, find_peaks_in_range_all_lengths
...
class PSitePredictor():
    ...
    def _cache_sequences(self):
        ...
        for tid, seq in tqdm(self.seq_dict.items(), mininterval=5):
            self._base_seq_emb[tid] = np.array([self.nt_mapping.get(base, 4) for base in seq.upper()], dtype=np.int8)
    ...
    def load_model(self, model_path: str, offset_limits: Tuple[int, int]):
        ...
        if offset_limits:
            self.offset_min = min(offset_limits)
            self.offset_max = max(offset_limits)
        elif self.offset_map:
            self.offset_min = min([v[0] for v in self.offset_map.values()])
            self.offset_max = max([v[1] for v in self.offset_map.values()])
        else:
    ...
    def evaluate_read_density(self,
        ...
        lengths, mat_starts, rel_pos_start = build_length_position_matrix(
            self.count_dict, self.tx_cds, output_dir,
    ...
    def evaluate_three_nucleotide_periodicity(
        ...
        lengths, mat, totals = build_length_frame_matrix(
            self.count_dict, self.tx_cds, output_dir, min_len, max_len, prefix)
    ...
    def calculate_offset_map_fix(
        ...
        mat_starts, mat_stops, rel_pos_start, rel_pos_stop, lengths = self.evaluate_read_density(output_dir, prefix="read")
        prd_read_len, lengths, totals = self.evaluate_three_nucleotide_periodicity(output_dir, prefix="read")
        ...
        # print(prd_read_len, inf_read_lens)
        # inf_rl_idx = [idx for idx, rl in enumerate(lengths) if rl in inf_read_lens]
        self.offset_fix = {
    ...
    def _prepare_train_datasets(self,
        ...
        print(f"Offset limits: {self.offset_min, self.offset_max}")
        ...
        for tid in tqdm(tids, desc=f"Generate train dataset around {train_ref}", mininterval=200):
            ...
            for pos in count:
                ...
                if not (offset >= self.offset_min and offset <= self.offset_max):
                    continue
                for read_l in count[pos]:
                    ...
                    if not offset_limits and self.offset_map:
        ...
        X_df = pd.DataFrame(X_seq, columns = [f'seq_{i}' for i in range(X_seq.shape[1])])
        X_df["read_len"] = pd.Categorical([rl - self.read_length_limits[0] for rl in read_len_list])
        self._feature_names_ = list(X_df.columns)

        y = np.array(offset_list, dtype=np.int64) - self.offset_min
    ...
    def _prepare_batch_datasets(self, 
        ...
        X_df = pd.DataFrame(X_seq, columns=[f for f in self._feature_names_ if f != 'read_len'])
        X_df['read_len'] = pd.Categorical([rl - self.read_length_limits[0] for rl in read_len_list])
    ...
    def _evaluate(self, y_true: np.ndarray, X_val: pd.DataFrame, output_dir: str):
        ...
        num_classes = self.offset_max - self.offset_min + 1
        report_labels = range(num_classes)
        target_names = [f"{i + self.offset_min}" for i in report_labels]
    ...
    def train(
        ...
        # }
        # cv_splits = [(range(0, X_train.shape[0]), range(X_train.shape[0], X_train.shape[0] + X_val.shape[0]))]
        ...
        # grid_result = grid_search.fit(pd.concat([X_train, X_val]), np.hstack([y_train, y_val]))
        # print("Best: %f using %s" % (grid_result.best_score_, grid_search.best_params_))
        # # save
        # pd.DataFrame(grid_result.cv_results_).to_csv(
        #     os.path.join(output_dir, "grid_search_results.csv"), header=True)
        ...
        DEFAULT_LGB_PARAMS = {
            'objective': 'multiclass',
            'num_class': num_classes,       # will be overwritten below to ensure correctness
            'learning_rate': 0.1,
            'n_estimators': 2000,
            'num_leaves': 32,
            'max_depth': 8,
        ...
        # Ensure n_jobs default is set if user didn't supply it
        if 'n_jobs' not in user_params:
            merged['n_jobs'] = self.n_thread
        ...
        with tqdm(total=total_tids, desc="Predicting P-site", mininterval=1) as pbar:
            while True:
                ...
                while start_idx < n_samples:
                    ...
                    offsets = np.array(pred_labels) + self.offset_min
...
if __name__ == "__main__":
    ...
    data_path = "/home/user/data3/yaoc"
    result_dir = "/home/user/data3/rbase/translation_model/results/p_site_pred/WM902B"
    ...
    preditor.load_read_count_data(
        os.path.join(data_path, "WM902B.read_count.pkl"),
        endonuclease = "MNase")
    time_i = time.time()
    print(f"[TIMING] load={time_i-time_s:.4f}s")

    preditor.train(
        output_dir = "/home/user/data3/rbase/translation_model/results/p_site_pred/WM902B/lightgbm",
        train_ref = "TTS",
        search_range = (-18, -12),
        lgb_params = {
                'learning_rate': 0.05,
    ...
    time_e = time.time()
    print(f"[TIMING] eval={time_e-time_i:.4f}s")
