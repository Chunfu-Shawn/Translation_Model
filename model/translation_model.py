...
from model.model_modules import LinearEmbedding, AddPromptEmbedding, EncoderLayer, Encoder, \
    MaskedSeqPredictorHead, MaskedCountPredictorHead
...
class TranslationModel(nn.Module):
    ...
    def forward(self, src_data_batch, tissue_idx, src_mask, head_names=["seq", "count"]):
        ...
        if "coding" in head_names:
            ...
            if "seq" in head_names:
                # predict masked sequence
                outputs.append(self.seq_head(src_reps))
            if "count" in head_names:
                # predict masked footprint
                outputs.append(self.count_head(src_reps))
            return outputs
