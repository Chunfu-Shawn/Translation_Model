...
def translate_dna_to_protein(sequence: str) -> str:
    """将 DNA 序列翻译为氨基酸序列（遇到 * 自动截断）"""
    protein = []
    # 确保序列长度是3的倍数，丢弃尾部不完整密码子
    for i in range(0, len(sequence) - len(sequence) % 3, 3):
        codon = sequence[i:i+3].upper().replace('U', 'T')
        aa = CODON_TABLE.get(codon, 'X')  # X 代表未知氨基酸
        if aa == '*':
            break
        protein.append(aa)
    return "".join(protein)
...
class TranslationSignalORFCaller:
    ...
    def run(self, 
        ...
        # =================================================================
        # Phase 6: 生成并保存高置信度多肽的 FASTA 文件 (使用 Biopython)
        # =================================================================
        if not final_df.empty:
            print("\n[Phase 6] Extracting and translating DNA sequences to Protein FASTA using Biopython...")
            fasta_path = os.path.join(out_dir, f"high_confidence_proteins.{self.cell_type}.{mode}_mode.fasta")
            ...
            with open(fasta_path, 'w') as f_out:
                for row in final_df.itertuples(index=False):
                    ...
                    if tid in self.seq_dict:
                        full_seq = self.seq_dict[tid]
                        
                        if start >= 0 and stop <= len(full_seq):
                            # 1. 切割出 ORF 的 DNA 序列
                            orf_seq = full_seq[start:stop+3]
                            
                            # 2. 使用 Biopython 翻译
                            # to_stop=True 表示遇到第一个终止密码子就停止翻译，且返回的序列不包含 '*'
                            try:
                                protein_seq = str(Seq(orf_seq).translate())
                            except Exception as e:
                                print(f"  [Warning] Translation failed for {tid}: {e}")
                                continue
                            
                            if protein_seq:
                                header = f">{tid}|{row.Cell_Type}|{start}:{stop}|Score:{row.score:.3f}"
                                
                                # 将长序列换行 (每行 80 个字符) 
                                split_prot = [protein_seq[i:i+80] for i in range(0, len(protein_seq), 80)]
                                
                                f_out.write(f"{header}\n")
                                f_out.write("\n".join(split_prot) + "\n")
                                extracted_count += 1

            print(f"  -> Successfully generated {extracted_count} protein sequences.")
