...
def plot_bar_sum_RPF_along_tx(data, block_start, block_end, output_file,
                          figsize=(12,2),
                          log_y=False, 
                          block_color='grey', 
                          block_alpha=0.15, 
    ...
    plt.ylabel('log2 RPF density' if log_y else 'RPF density', fontsize=8)

    x_positions = range(0, tx_len)

    axs.bar(x_positions, data_np, width=1, color='#08306B', zorder=2)
    if draw_block:
        axs.axvspan(bs, be, ymin=0, ymax=1, color=block_color, alpha=block_alpha, zorder=block_zorder)
    axs.set_title("All RPF length", fontsize=10, pad=1)
