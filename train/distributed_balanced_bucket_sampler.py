...
#         cell_type_idxs (List[int]): 每个样本对应的细胞类型 index，用于平衡采样
#         balance_classes (bool): 是否开启类别平衡（自动下采样多数量的类别）
#     """
#     def __init__(self,
#                  lengths,
#                  batch_size,
...
#                  drop_last=False,
#                  seed=0,
#                  cell_type_idxs=None,      # [新增参数]
#                  balance_classes=False):   # [新增参数]
...
#             if cell_type_idxs is None:
#                 raise ValueError("cell_type_idxs must be provided when balance_classes=True")
            
#             # 统计每种细胞类型的数量
#             counts = Counter(cell_type_idxs)
#             self.min_count = min(counts.values())
#             self.classes = list(counts.keys())
            
#             # 将索引按类别分组，以备在 __iter__ 中按 epoch 动态抽取
#             self.class_to_indices = {c: [] for c in self.classes}
#             for idx, c in enumerate(cell_type_idxs):
#                 self.class_to_indices[c].append(idx)
            
#             # 计算平衡后的理论总样本数 (最小类别数 * 类别总数)
...
#         # 计算全局需要的样本总数，用于后续 padding 防止 DDP 卡死
...
#             # 截断（通常在使用 drop_last=True 时触发）
#             selected_indices = selected_indices[:self.total_size]

#         # ---------------------------------------------------------
#         # 3. 继续你原有的 Bucket 逻辑，但针对的是过滤后的 selected_indices
#         # ---------------------------------------------------------
#         # 3.1 sort by length
...
#                 # 由于前面已经做了 padding/truncating，这里理论上不需要 continue，但保留以防万一
...
#         # 随机打乱 Batch 的顺序 (保证长度相近的 Batch 不会永远在一起)
#         if self.shuffle:
