...
class PretrainingTrainer:
    ...
    def count_task_criterion(
        ...
        if pred_shape.shape[2] == 1:
            shape_loss_all = self.count_criterion(pred_shape.squeeze(-1), count_raw_emb.squeeze(-1)) * count_emb_masks.float()
        else:
            shape_loss_all = self.count_criterion(pred_shape, count_raw_emb) * count_emb_masks.unsqueeze(-1).float()
