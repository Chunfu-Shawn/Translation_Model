...
class FineTuningTrainer:
    ...
    def __init__(
        ...
        self.pos_loss = nn.CrossEntropyLoss(weight=torch.Tensor([20,20,1]).to(self.device), ignore_index=-1, reduction='mean')
        self.alpha = 0.5
        self.gamma = 2.0
    ...
    def compute_class_weights_from_dataset(label_indices_tensor, num_classes=3, eps=1e-9):
        """
        label_indices_tensor: 1D tensor of all labels (flattened) from training set, excluding pad/ignored.
        returns weight tensor shape (num_classes,)
        Common choice: weight[c] = total_count / (num_classes * count[c])
        """
        counts = torch.bincount(label_indices_tensor, minlength=num_classes).float()
        total = counts.sum()
        # avoid div by zero
        counts = torch.where(counts == 0.0, torch.ones_like(counts), counts)
