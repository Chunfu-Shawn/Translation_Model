...
# def extract_initiation_features(preds, seqs):
#     """
#     遍历预测结果，提取起始密码子附近的特征。
...
#         pred_arr = np.expm1(sample['pred'].reshape(-1))
...
def extract_initiation_features(preds, seqs):
    """
    Iterate over prediction results to extract features around start codons.
    [MODIFIED] Adapted to handle nested dict format: {cell_type: {tid: prediction}}
    """
    results = []
    target_codons = {'ATG', 'CTG', 'TTG', 'GTG'}
    ...
    for cell_type, tid_dict in preds.items():
        ...
        for tid, pred_signal in tqdm(tid_dict.items(), leave=False):
            ...
            # 2. Format Prediction Array safely (handling PyTorch tensors if present)
            if hasattr(pred_signal, 'cpu'):
                pred_signal = pred_signal.cpu().numpy()
            
            # Inverse log1p transformation to restore linear space
            pred_arr = np.expm1(np.array(pred_signal).reshape(-1).astype(np.float32))
            
            limit = min(len(pred_arr), seq_len)
            global_mean = np.mean(pred_arr[:limit]) + 1e-6
            
            # 3. Scan sequence for Motifs
            for i in range(6, limit - 5):
                codon = seq_str[i : i+3]
                ...
                if codon in target_codons:
                    ...
                    # [MODIFIED] Replace UUID with explicit Tid and Cell_type
                    results.append({
                        'Tid': clean_tid,
                        'Cell_type': cell_type,
                        'Start codon': codon,
...
from plotnine import *
from scipy.stats import ranksums
from scipy.stats import spearmanr # [NEW] 确保导入了 spearmanr

def analyze_and_plot_initiation(meta_df, out_dir, suffix=""):
    """
    Perform statistical tests and plot Kozak feature distribution.
    """
    if meta_df.empty:
        print("DataFrame is empty, skipping plot and stats.")
        return
    ...
    # 动态计算 Y 轴上限，防止长尾离群点把图拉得太高
    y_limit = df_plot['Normalized_Density'].quantile(0.99) * 1.2
    
    # =================================================================
    # [NEW] 计算全局的 Spearman 相关性 (Kozak score vs Density)
    # =================================================================
    ...
    # 创建一个用于标注的极小 DataFrame，X=4.4 刚好在图表的右上方
    anno_df = pd.DataFrame({'x': [4.4], 'y': [y_limit * 0.95], 'label': [corr_label]})
    
    # --- A. Plotting ---
    p = (
        # [MODIFIED] 将 color 映射移除了，放到箱线图中，以免干扰全局趋势线
        ggplot(df_plot, aes(x='Start codon', y='Normalized_Density'))
        
        # [MODIFIED] 在 geom_boxplot 内部单独映射 color，设置白底彩边
        + geom_boxplot(
            aes(color='Kozak class'), 
            fill='white',               # 内部填充纯白
            alpha=1.0,
            width=0.7,
            size=0.8,                   # 边框和中位线加粗，提升质感
            outlier_shape=None,         # [CRITICAL] 彻底阻止离群点渲染，解决 13MB 的 PDF 体积爆炸问题
            outlier_alpha=0,
            outlier_size=0,
        ...
        # =================================================================
        # [NEW] 添加一条贯穿全局的虚线拟合趋势线，置于底层
        # group=1 强制将所有 x 轴类别当成一个整体进行线性拟合 (lm)
        # =================================================================
        + geom_smooth(
            aes(group=1), 
            method='lm', 
        ...
        + labs(
            y="Predicted signal peak strength"
        )
        # 对应顺序的颜色：深蓝 -> 中蓝 -> 浅蓝 -> 浅灰
        + scale_color_manual(values=["#08306B", "#2171B5", "#6BAED6", "#B0B0B0"]) 
        
        # 使用 coord_cartesian 截断过高的 Y 轴
        + coord_cartesian(ylim=(-0.01, y_limit))
    )
    
    plot_filename = f"start_codon_kozak_boxplot.{suffix}.pdf" if suffix else "start_codon_kozak_boxplot.pdf"
    plot_path = os.path.join(out_dir, plot_filename)
    p.save(plot_path, width=5, height=5, verbose=False)
...
# [MODIFIED] Removed the default argument ratio_mask=1.0 to match downstream definitions
def evaluate_start_codon_kozak_motif(pred_pkl, seq_pkl, out_dir="./results/initiation_eval", suffix=""):
    """
    ...
    # Step 1: Feature Extraction (Removed the non-existent ratio_mask kwarg)
    df_features = extract_initiation_features(preds, seqs)
