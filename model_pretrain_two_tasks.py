...
class PretrainingTrainer:
    def __init__(self,
        ...
        self.count_criterion = nn.MSELoss(reduction="none") # for count nn.SmoothL1Loss(reduction="none", beta=1)
    ...
    def dynamic_weight_average(self, epoch, T = 2.0, eps = 1e-8):
        ...
        if epoch > 1:
            ...
            w_i = prev / (prev2 + eps)
            loss_weight = torch.tensor([5, 1], device=device) * F.softmax(w_i / T, dim=-1) # default weight for seq, count and tissue is 5, 1
        else:
            loss_weight = torch.tensor([5, 1], device=device)  # torch.ones(self.num_losses, device=device) # torch.ones_like(losses).cuda()
    ...
    def train(self, epoch):
        ...
        for batch_idx, batch_data in enumerate(self.dataloader):
            ...
            with torch.amp.autocast(device_type='cuda', dtype=torch.float16):
                output = self.model(inps, tissue_idx, pad_masks, ["seq", "count"])
                # multi-task learning losses
                losses = self.multi_task_criterion(output, raw_embs, emb_masks)
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch_data in enumerate(self.val_loader):
                raw_emb, inp, emb_mask, pad_mask, tissue_idx = [data.cuda(non_blocking=True) for data in batch_data]
                 ...
                 # run the forward pass with autocasting
                with torch.amp.autocast(device_type='cuda', dtype=torch.float16):
                    output = self.model(inp, tissue_idx, pad_mask, ["seq", "count"])
                    losses = self.multi_task_criterion(output, raw_emb, emb_mask)
